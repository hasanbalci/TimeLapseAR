...
if useGroundProb
    ...
    classifiersPath = getPathName('codeUtils', 'GeometricContext', 'data');
...
figure(3); imshow(img);
displayBoundaries(figure(3), boundaries(boundaryLabels==0), 'b', 2);
groundStr = {'Shadows', 'Ground shadows'};
%title(sprintf('%s detected', groundStr{useGroundProb+1}));
%val = cellfun(@(x) length(x), boundaries(boundaryLabels==0));
%longestBoundary = (val==max(val));
%disp(longestBoundary);
imwrite(img, 'timelapseWithRedLine.jpg');
